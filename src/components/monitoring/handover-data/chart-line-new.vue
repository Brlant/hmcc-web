...
<template>
  <div>
    ...
    <div v-else id="chartLine" class="chart-Line" :style="{width: chartWidth}"></div>
  </div>
</template>
<script>
  const unitAry = ['', '°C', '%', 'V'];
  const titleAry = ['', '温度', '湿度', '电压'];
  import {HandoverData} from '@/resources';
  ...
  export default {
    ...
    methods: {
      ...
      queryList() {
        const {startTime, endTime, devNo, devCode, valType, startPrice} = this.filters;
        ...
        typeList.forEach((i, index) => {
          const params = {startTime, endTime, devNo, devCode, valType: i, startPrice};
          httpAry.push(HandoverData.queryHandoverDataByDevNo(devCode, params));
        });
        ...
        axios.all(httpAry)
          .then(axios.spread((...args) => {
            ...
            typeList.forEach((i, index) => {
              const data = args[index].data && args[index].data.map(m => {
                return {
